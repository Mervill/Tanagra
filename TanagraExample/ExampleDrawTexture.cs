...
{
    public class ExampleDrawTexture : ExampleBase
    {
        class UniformData
        {
        ...
        public ExampleDrawTexture()
        {
            ...
            for(int x = 0; x < swapchainData.Images.Count(); x++)
                CreateCommandBuffer(cmdBuffers[x], vertexData, renderPass, pipelineLayout, pipeline, descriptorSet, swapchainData.Images[x], swapchainData.Framebuffers[x]);
