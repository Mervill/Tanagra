...
namespace Tanagra.Generator
{
    public class CSharpCodeGenerator
    {
        ...
        readonly List<string> unhiddenCountMembers;
        ...
        public CSharpCodeGenerator()
        {
            ...
            unhiddenCountMembers = new List<string>
            {
        ...
        string GenerateManagedClass(VkStruct vkStruct)
        {
            ...
            foreach(var vkMember in vkStruct.Members)
            {
                ...
                var count = vkStruct.Members.Count(x => x.Name == memberCountName);
                if(count == 1 && !hiddenMembers.Contains(vkStruct.Name) && !unhiddenCountMembers.Contains($"{vkStruct.Name}.{memberCountName}"))
            ...
            foreach(var vkMember in vkStruct.Members.Where(x => !hiddenMembers.Contains(x.Name)))
            {
                if(vkMember.Type.Name == "String")
                {
                    ...
                    {
                        WriteMemberStringArray(vkStruct, vkMember, vkStruct.ReturnedOnly);
                    }
                    else
                    {
                ...
                if(vkMember.IsArray)
                {
                    ...
                    WriteMemeberArray(vkStruct, vkMember, vkStruct.ReturnedOnly);
                    WriteLine("");
                    continue;
                }
        ...
        void WriteMemberStringArray(VkStruct vkStruct, VkMember vkMember, bool readOnly)
        {
        ...
        void WriteMemeberArray(VkStruct vkStruct, VkMember vkMember, bool readOnly)
        {
            ...
            var countMember = vkStruct.Members.Where(x => x.Name == countName).First();
            ...
            // struct
            {
                var memberStruct = vkMember.Type as VkStruct;
                var structType = memberStruct.Name;
                if(IsManagedStruct(memberStruct))
                    structType = $"{UnmanagedNS}." + structType;

                var getValueCast = IsManagedStruct(memberStruct) ? $"new {vkMember.Type}(&ptr[x])" : "ptr[x]";
        ...
        void WriteArray(VkMember vkMember, string countName, bool readOnly, string structType, string sizeType, string pointerType, string get, string set, bool intPtr = false)
        {
            ...
            _tabs--;
            if(intPtr)
                WriteLine($"var valueCount = (Int32){NativePointer}->{countName};");
            else
                ...
                if(commandInfo.HasArrayParams)
                {
                    ...
                    foreach(var kv in commandInfo.ParamListCountMap)
                    {
                        ...
                        if(StackallocListArgs)
                        {
                            ...
                            WriteLine($"var {ptrVar} = stackalloc {arrayType}[(Int32){countName}];");
                            //WriteLine($"if({countName} != 0)");
        ...
        string CreateCommandCallParams(List<VkParam> vkCmdParams, CommandInfo commandInfo, bool isSecondArrayCall)
        {
            ...
            foreach(var vkParam in vkCmdParams)
            {
                ...
                if(IsPlatformStruct(paramType) || paramType is VkEnum)
                {
                    ...
                    if(vkParam.Type.Name == "IntPtr" && commandInfo.ParamListCountMap.ContainsValue(vkParam))
                    {
                        internalCallParams.Add($"new IntPtr({paramName})");
                    }
                    else
                    {
            ...
            foreach(var vkHandle in vkHandles)
            {
                ...
                foreach(var vkCommand in handleCommands)
                {
                    ...
                    if(commandName.StartsWith("Cmd"))
                        commandName = commandName.Substring(3, commandName.Length - 3);
